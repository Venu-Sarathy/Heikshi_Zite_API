...
cookie_file = "/home/venu/telegram_api/cookies"
authorization_file = "/home/venu/telegram_api/auth_token"
...
def get_enctoken():
    ...
    try:
        ...
        f.close()
        with open(authorization_file, 'r') as au:
            auth_token = au.read().strip()
            session.headers.update({'Authorization': f'enctoken {auth_token}'})
        au.close()
        print("Trying to Connect with Previous Session ...")
        instruments = 'NSE:INFY'
        if session.get(f"https://api.kite.trade/quote/ltp", params={"i": instruments}).status_code == 200:
            print("Success ...")
        ...
        response = session.post('https://kite.zerodha.com/api/login', data={
            "user_id": parser['Zerodha']['user_id'],
            "password": parser['Zerodha']['password']
        })
        response = session.post('https://kite.zerodha.com/api/twofa', data={
            "request_id": response.json()['data']['request_id'],
            "twofa_value": input("Two Factor Code : "),
...
class KiteApp:
    ...
    def __init__(self):
        ...
        #self.session.headers.update({'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.5735.134 Safari/537.36', 'X-Kite-Userid': 'FVB175'})

        self.root_url = "http://api.kite.trade"
        ...
        f.close()
        with open(authorization_file, 'w') as au:
            au.write(enctoken)
        au.close()
    ...
    def ltp(self, instruments):
        data = self.session.get(f"{self.root_url}/quote/ltp", params={"i": "NFO:"+instruments}, headers=self.headers).json()["data"]['NFO:'+instruments]
        return data

    def ltp_other(self, instruments):
        data = self.session.get(f"{self.root_url}/quote/ltp", params={"i": instruments}, headers=self.headers).json()["data"][instruments]
        return data

    def ltp_ex(self, tradingsymbol, exchange="NFO", transaction_type="BUY", variety="regular", product="NRML", order_type="MARKET"):
        pass
    ...
    def place_order(self,  tradingsymbol,  price, transaction_type, quantity , order_type,  exchange="NFO", product="NRML",
                    variety="regular",validity="DAY", disclosed_quantity=None, trigger_price=None, squareoff=None, stoploss=None,
    ...
    def modify_order(self, order_id, price, quantity, order_type="SL", variety="regular", product="NRML", parent_order_id=None,transaction_type="SELL",
                     trigger_price=None, validity="DAY", disclosed_quantity=None):
        params = locals()
        del params["self"]
        for k in list(params.keys()):
            if params[k] is None:
        ...
        #order_id = self.session.put(f"https://kite.zerodha.com/oms/orders/{variety}/{order_id}",
    ...
    def cancel_order(self,  order_id, variety="regular", parent_order_id=None):
        #order_id = self.session.delete(f"https://kite.zerodha.com/oms/orders/{variety}/{order_id}",
